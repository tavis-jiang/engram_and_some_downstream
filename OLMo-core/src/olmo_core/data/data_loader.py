...
class UBDataLoader(DataLoaderBase):
    def __init__(
        self,
        ...
        super().__init__(
            work_dir="",
            global_batch_size=ub_global_batch_size * seq_len,
            dp_world_size=dp_world_size,
            dp_rank=dp_rank,
            fs_local_rank=fs_local_rank,
        )
        # the global batch size and rank microbatch size see UBDataLoaderConfig
        self.local_path = local_path
        self.remote_path = remote_path
        self.proportion = proportion

        self.seq_len = seq_len
        self.ub_global_batch_size = ub_global_batch_size

        self.tokenizer_path = tokenizer_path
        self.ckpt_path = ckpt_path

        self.consumed_samples = 0
        self.data_loader: Optional[TokenStreamDataLoader] = None
    ...
    def _init_data_loader(self, consumed_samples: int):
        log.info("Initializing data loader for UBDataLoader")
        assert self.data_loader is None
        # NOTE: ONLY SUPPORT THE DP PARALLEL MODE
        self.data_loader = TokenStreamDataLoader(
            local_path=self.local_path,
            remote_path=self.remote_path,
            proportion=self.proportion,
            seq_len=self.seq_len,
            consumed_samples=consumed_samples,
            micro_batch_size=self.ub_global_batch_size // self.dp_world_size,
            data_parallel_rank=self.dp_rank,
            data_parallel_size=self.dp_world_size,
            tokenizer_path=self.tokenizer_path,
            ckpt_path=self.ckpt_path,
            text_chunk_queue_size=self.text_chunk_queue_size,
            text_chunk_size=self.text_chunk_size,
            prefetch_queue_size=self.prefetch_queue_size,
            use_token_column=self.use_token_column,
    ...
    def global_num_tokens_in_batch(self, batch: Dict[str, Any]) -> Optional[int]:
        return self.ub_global_batch_size * self.seq_len
    ...
    def state_dict(self) -> Dict[str, Any]:
        if self.data_loader is None:
            return {"consumed_samples": 0}
        return {"consumed_samples": self.data_loader.consumed_samples}
    ...
    def get_mock_batch(self) -> Dict[str, Any]:
        device = torch.device("cpu")
        rng = torch.Generator(device=device)
        rng.manual_seed(42 + self.dp_rank)
        num_instances = self.ub_global_batch_size

        input_ids = torch.randint(
            0,
            1000,
            (num_instances, self.seq_len),
            generator=rng,
            device=device,
        )
        out: Dict[str, Any] = {"input_ids": input_ids}
...
@dataclass
class UBDataLoaderConfig(Config):
    """
    A configuration class for building :class:`UBDataLoaderBase` data loaders.
    """
    ...
    # NOTE: in olmo, the global batch size is the total number of tokens in the batch
    # the rank microbatch size is the number of tokens in the each rank
    # the gradient accumulation steps is the number of tokens dataloader will give divided by the rank microbatch size
    # but in ubdataloader, the global batch size is the number of samples in the batch
    # the rank microbatch size is the number of samples in the each rank
    # so ubdataloader we set the micro batch size to be the global batch size / dp world size
    ...
    def build(
        ...
        """
        Construct the :class:`UBDataLoaderBase`.

        :param data_path: The data path. like below:
        ...
        for weight, local, remote in zip(
            self.data_path[::3], self.data_path[1::3], self.data_path[2::3]
        ):
        ...
        return UBDataLoader(
            ...
            dp_rank=dp_rank,
            fs_local_rank=get_fs_local_rank(),
            ub_global_batch_size=self.ub_global_batch_size,
            tokenizer_path=self.tokenizer_path,
            text_chunk_queue_size=self.text_chunk_queue_size,
            text_chunk_size=self.text_chunk_size,
            prefetch_queue_size=self.prefetch_queue_size,
