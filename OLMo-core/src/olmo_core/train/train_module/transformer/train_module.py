...
from .metrics import (
    ...
    collect_grad_norms,
    extract_batch_bytes,
    filter_grad_norm_keys,
    set_moe_metrics_enabled,
)
...
class TransformerTrainModule(TrainModule):
    ...
    def __init__(
        ...
        metrics: Optional[TransformerMetricsConfig] = None,
        ...
        # Configure metric tracking before enabling MoE-specific logging.
        self.metrics_config = metrics
        self._metrics_enabled = bool(metrics and metrics.enabled)
        self._activation_trackers: Dict[str, ActivationTracker] = {}
        self._loss_per_bytes_warned = False

        set_moe_metrics_enabled(
            self.model,
            self._metrics_enabled
            and self.metrics_config is not None
            and self.metrics_config.log_moe_metrics,
        )
        self._initialize_activation_trackers()
    ...
    def _initialize_activation_trackers(self) -> None:
        for tracker in self._activation_trackers.values():
            tracker.close()
        self._activation_trackers = {}
        if not self._metrics_enabled or self.metrics_config is None:
            return
        if (
            self.metrics_config.log_last_activation_norm
            and getattr(self.model, "lm_head", None) is not None
        ):
            self._activation_trackers["activation_norm/last_hidden"] = ActivationTracker(
                self.model.lm_head, pre_hook=True
            )
        if (
            self.metrics_config.log_embedding_norm
            and getattr(self.model, "embeddings", None) is not None
        ):
            self._activation_trackers["activation_norm/embedding"] = ActivationTracker(
                self.model.embeddings, pre_hook=False
            )

    def _reset_activation_trackers(self) -> None:
        for tracker in self._activation_trackers.values():
            tracker.reset()

    def _log_activation_metrics(self) -> None:
        if not self._metrics_enabled or not self._activation_trackers:
            return
        for base_name, tracker in self._activation_trackers.items():
    ...
    def _log_batch_metrics(
        ...
        *,
        ...
        ce_batch_loss: torch.Tensor,
    ) -> None:
        if not self._metrics_enabled or self.metrics_config is None:
            return
        cfg = self.metrics_config
        tokens_for_loss = batch_num_tokens_for_loss.detach().float()
        if cfg.log_batch_size:
            self.record_metric(
                "batch size",
        ...
        if cfg.log_effective_length and batch_size > 0:
            effective_length = tokens_for_loss / float(batch_size)
            self.record_metric(
                "effective length",
                effective_length,
                ReduceType.mean,
        ...
        if cfg.log_loss_per_byte:
            if total_bytes and total_bytes > 0:
                loss_per_byte = ce_batch_loss.detach() * (tokens_for_loss / float(total_bytes))
                self.record_metric(
                    "loss per byte",
                    loss_per_byte,
                    ReduceType.mean,
            ...
            elif not self._loss_per_bytes_warned:
                log_once(
                    log,
                    "Skipping loss_per_byte metric because batch metadata does not include byte or character counts.",
                )
                self._loss_per_bytes_warned = True

    def _log_gradient_metrics(self) -> None:
        if (
            not self._metrics_enabled
            or self.metrics_config is None
        ):
            return
        if self.metrics_config.log_param_mean:
            self._log_param_means()
        if not (
            self.metrics_config.log_layer_grad_norm or self.metrics_config.log_module_grad_norm
        ):
            return
        norms = collect_grad_norms(
            self.model,
            include_layerwise=self.metrics_config.log_layer_grad_norm,
            include_modulewise=self.metrics_config.log_module_grad_norm,
        )
        ...
        if self.metrics_config.log_module_grad_norm:
            summary = filter_grad_norm_keys(norms)
        self._log_grad_norm_summary(summary)

    def _log_grad_norm_summary(
        self, summary: Dict[str, float], *, stage_prefix: Optional[str] = None
    ) -> None:
        if (
            not summary
            or not self._metrics_enabled
            or self.metrics_config is None
            or not self.metrics_config.log_module_grad_norm
        ):
            return
        step = getattr(getattr(self, "trainer", None), "global_step", None)
        prefix = stage_prefix or "Transformer"
        summary_str = ", ".join(f"{key}={value:.6f}" for key, value in summary.items())
        if step is not None:
            log.info("%s grad norms at step %d: %s", prefix, step, summary_str)
        else:
            log.info("%s grad norms: %s", prefix, summary_str)

    def _log_param_means(self) -> None:
        if (
            not self._metrics_enabled
            or self.metrics_config is None
            or not self.metrics_config.log_param_mean
        ):
            return
        keywords = tuple(self.metrics_config.param_mean_keywords)
        if not keywords:
            return
        means = collect_param_means(self.model, keywords=keywords)
    ...
    def train_batch(self, batch: Dict[str, Any], dry_run: bool = False):
        ...
        if self._activation_trackers:
            self._reset_activation_trackers()
        ...
        if (
            self._metrics_enabled
            and self.metrics_config is not None
            and self.metrics_config.log_loss_per_byte
        ):
            total_bytes = extract_batch_bytes(
                batch.get("metadata"),
                preferred_keys=self.metrics_config.metadata_byte_keys,
            )
        ...
        mort_sparse_aux_loss_total: Optional[torch.Tensor] = None
        ...
        for micro_batch_idx, micro_batch in enumerate(micro_batches):
            with self._train_microbatch_context(micro_batch_idx, num_micro_batches):
                ...
                mort_sparse_aux_loss = self._collect_mort_sparse_aux_loss()
                if mort_sparse_aux_loss is not None:
                    loss = loss + mort_sparse_aux_loss
                    if mort_sparse_aux_loss_total is None:
                        mort_sparse_aux_loss_total = move_to_device(torch.tensor(0.0), self.device)
                    mort_sparse_aux_loss_total += get_local_tensor(mort_sparse_aux_loss.detach())
        ...
        if mort_sparse_aux_loss_total is not None:
            mort_sparse_aux_loss_avg = mort_sparse_aux_loss_total / num_micro_batches
            if is_distributed():
                dist.all_reduce(mort_sparse_aux_loss_avg)
                mort_sparse_aux_loss_avg.div_(self.world_size)
            self.record_metric(
                "mort_sparse_aux_loss",
                mort_sparse_aux_loss_avg,
                ReduceType.mean,
                namespace="train",
            )
    ...
    def model_forward(
        ...
        with self._model_forward_context():
            step = getattr(getattr(self, "trainer", None), "global_step", None)
            return self.model(input_ids, labels=labels, step=step, **kwargs)

    def _collect_mort_sparse_aux_loss(self) -> Optional[torch.Tensor]:
        total_aux_loss: Optional[torch.Tensor] = None
        for block in self.model.blocks.values():
            core_block = getattr(block, "_checkpoint_wrapped_module", block)
            aux = getattr(core_block, "mort_sparse_aux_loss", None)
            is_mort_sparse = (
                getattr(core_block, "mort_sparse_weight_generator", None) is not None
                and getattr(core_block, "mort_sparse_scaler", None) is not None
            )
            if aux is None:
                if is_mort_sparse:
                    log.warning(
                        "[train_module] MORT_sparse block %s missing mort_sparse_aux_loss this micro-batch",
                        getattr(core_block, "block_idx", "unknown"),
                    )
                continue
            ...
            core_block.mort_sparse_aux_loss = None
        return total_aux_loss

    def num_flops_per_token(self, seq_len: int) -> int:
        model = self.model
        # FSDP/DDP wrappers may not expose num_flops_per_token; unwrap common attributes.
        if not hasattr(model, "num_flops_per_token"):
            for attr in ("module", "_fsdp_wrapped_module", "_orig_module"):
                inner = getattr(model, attr, None)
                if inner is not None and hasattr(inner, "num_flops_per_token"):
                    model = inner
                    break
        if not hasattr(model, "num_flops_per_token"):
            raise AttributeError(f"{type(self.model).__name__} has no num_flops_per_token")
